...
struct ManifestWriteFailureState {
  ...
  int throw_on_manifest_close = 0;
  std::vector<std::string> manifest_outputs;
  std::vector<std::string> manifest_close_attempts;
  std::vector<std::string> deleted_files;
  std::unordered_set<std::string> open_manifest_paths;
  std::vector<std::string> deleted_open_files;
};
...
class ManifestWriteFailureOutputStream : public PositionOutputStream {
 public:
  ManifestWriteFailureOutputStream(std::unique_ptr<PositionOutputStream> delegate,
    ...
    state_->open_manifest_paths.insert(location_);
  }
  ...
  Status Close() override {
    ...
    state_->open_manifest_paths.erase(location_);
    if (state_->manifest_close_count == state_->throw_on_manifest_close) {
      throw std::runtime_error("Injected rewrite manifest close exception");
    }
...
class ManifestWriteFailureFileIO : public FileIO {
 public:
  ...
  Status DeleteFile(const std::string& file_location) override {
    if (state_->open_manifest_paths.contains(file_location)) {
      state_->deleted_open_files.push_back(file_location);
    }
...
class RewriteManifestsTest : public MinimalUpdateTestBase,
 ...
 protected:
  ...
  Result<std::vector<ManifestFile>> CurrentManifests() {
    ...
    SnapshotReader reader(snapshot.get());
    ICEBERG_ASSIGN_OR_RAISE(auto manifests, reader.DataManifests(file_io_));
  ...
  Result<std::vector<ManifestFile>> CurrentDeleteManifests() {
    ...
    ICEBERG_ASSIGN_OR_RAISE(auto snapshot, table_->current_snapshot());
    SnapshotReader reader(snapshot.get());
    ICEBERG_ASSIGN_OR_RAISE(auto manifests, reader.DeleteManifests(file_io_));
    return std::vector<ManifestFile>(manifests.begin(), manifests.end());
  }
...
TEST_P(RewriteManifestsTest, ClosesWritersWhenClusterByThrows) {
  ASSERT_THAT(AppendFiles({file_a_, file_b_, file_c_}), IsOk());
  ICEBERG_UNWRAP_OR_FAIL(auto before, CurrentManifests());

  for (bool explicit_transaction : {false, true}) {
    for (int close_failure : {0, 1, 2}) {
      SCOPED_TRACE(std::format("explicit_transaction={}, close_failure={}",
                               explicit_transaction, close_failure));
      auto state = std::make_shared<ManifestWriteFailureState>();
      state->fail_on_manifest_close = close_failure == 1 ? 1 : 0;
      state->throw_on_manifest_close = close_failure == 2 ? 1 : 0;
      BindTableWithFileIO(std::make_shared<ManifestWriteFailureFileIO>(
          file_io_, table_location_ + "/metadata/", state));

      std::shared_ptr<Transaction> txn;
      std::shared_ptr<RewriteManifests> rewrite;
      if (explicit_transaction) {
        ICEBERG_UNWRAP_OR_FAIL(txn, table_->NewTransaction());
        ICEBERG_UNWRAP_OR_FAIL(rewrite, txn->NewRewriteManifests());
      } else {
        ICEBERG_UNWRAP_OR_FAIL(rewrite, table_->NewRewriteManifests());
      }
      rewrite->ClusterBy([calls = 0](const DataFile& file) mutable -> std::string {
        if (++calls == 3) {
      ...
      EXPECT_THAT(result, HasErrorMessage("Injected cluster failure"));
      if (txn) {
        EXPECT_EQ(txn->state(), TransactionState::kFailed);
      }
      ASSERT_EQ(state->manifest_outputs.size(), 2U);
      EXPECT_THAT(state->manifest_close_attempts,
                  ::testing::UnorderedElementsAreArray(state->manifest_outputs));
      EXPECT_TRUE(state->open_manifest_paths.empty());
      EXPECT_TRUE(state->deleted_open_files.empty());
      for (const auto& path : state->manifest_outputs) {
        EXPECT_THAT(state->deleted_files, ::testing::Contains(path));
...
TEST_P(RewriteManifestsTest, CleansWritersAfterTransactionApplyFailure) {
  ASSERT_THAT(AppendFiles({file_a_, file_b_, file_c_}), IsOk());
  ICEBERG_UNWRAP_OR_FAIL(auto before, CurrentManifests());
  auto state = std::make_shared<ManifestWriteFailureState>();
  state->fail_on_manifest_output = 3;
  BindTableWithFileIO(std::make_shared<ManifestWriteFailureFileIO>(
      file_io_, table_location_ + "/metadata/", state));

  ICEBERG_UNWRAP_OR_FAIL(auto txn, table_->NewTransaction());
  ICEBERG_UNWRAP_OR_FAIL(auto rewrite, txn->NewRewriteManifests());
  rewrite->ClusterBy([](const DataFile& file) { return file.file_path; });
  EXPECT_THAT(rewrite->Commit(), IsError(ErrorKind::kIOError));
  EXPECT_EQ(txn->state(), TransactionState::kFailed);
  ASSERT_EQ(state->manifest_outputs.size(), 3U);
  EXPECT_THAT(state->manifest_close_attempts,
              ::testing::UnorderedElementsAre(state->manifest_outputs[0],
                                              state->manifest_outputs[1]));
  EXPECT_TRUE(state->open_manifest_paths.empty());
  EXPECT_TRUE(state->deleted_open_files.empty());
  for (const auto& path : state->manifest_outputs) {
    EXPECT_THAT(state->deleted_files, ::testing::Contains(path));
...
TEST_P(RewriteManifestsTest, ClosesAndCleansCopiedManifestOnFailure) {
  if (format_version() != 1) {
    GTEST_SKIP() << "Manifest copying is only required without snapshot ID inheritance";
  }
  ASSERT_THAT(AppendFiles({file_a_}), IsOk());
  ...
  ICEBERG_UNWRAP_OR_FAIL(
      ...
      WriteAddedManifest("copy-invalid-entry", {file_a_}, snapshot->snapshot_id));
  // Counts pass AddManifest validation; reading the entries must still reject it.
  invalid_manifest.added_files_count = 0;
  invalid_manifest.existing_files_count = 1;
  ICEBERG_UNWRAP_OR_FAIL(
      auto valid_manifest,
      WriteExistingManifest("copy-close-failure", snapshot->snapshot_id, {file_a_}));

  for (bool explicit_transaction : {false, true}) {
    for (bool invalid_entry : {false, true}) {
      SCOPED_TRACE(std::format("explicit_transaction={}, invalid_entry={}",
                               explicit_transaction, invalid_entry));
      auto state = std::make_shared<ManifestWriteFailureState>();
      state->fail_on_manifest_close = invalid_entry ? 0 : 1;
      BindTableWithFileIO(std::make_shared<ManifestWriteFailureFileIO>(
          file_io_, table_location_ + "/metadata/", state));
      std::shared_ptr<Transaction> txn;
      std::shared_ptr<RewriteManifests> rewrite;
      ...
      const auto& source = invalid_entry ? invalid_manifest : valid_manifest;
      rewrite->DeleteManifest(before[0]).AddManifest(source);
      auto result = rewrite->Commit();
      EXPECT_THAT(result, IsError(invalid_entry ? ErrorKind::kValidationFailed
                                                : ErrorKind::kIOError));
      EXPECT_THAT(result, HasErrorMessage(
                              invalid_entry ? "Cannot add manifest with added files"
                                            : "Injected rewrite manifest close failure"));
      if (txn) {
        EXPECT_EQ(txn->state(), TransactionState::kFailed);
      }
      ASSERT_EQ(state->manifest_outputs.size(), 1U);
      EXPECT_THAT(state->manifest_close_attempts,
                  ::testing::ElementsAre(state->manifest_outputs[0]));
      EXPECT_TRUE(state->open_manifest_paths.empty());
      EXPECT_TRUE(state->deleted_open_files.empty());
      EXPECT_THAT(state->deleted_files, ::testing::Contains(state->manifest_outputs[0]));
      EXPECT_FALSE(FileExists(state->manifest_outputs[0]));
      EXPECT_TRUE(FileExists(source.manifest_path));
      ICEBERG_UNWRAP_OR_FAIL(auto after, CurrentManifests());
      EXPECT_EQ(after, before);
    }
  }
...
TEST_P(RewriteManifestsTest, AppendDuringRewriteManifest) {
  ...
  // Retry cleanup discards the staged rewrite and reclusters the latest manifests.
  ICEBERG_UNWRAP_OR_FAIL(auto manifests, CurrentManifests());
  ASSERT_EQ(manifests.size(), 1U);
  ExpectManifestEntriesWithSnapshotIds(
      manifests[0], {file_a_->file_path, file_b_->file_path},
...
TEST_P(RewriteManifestsTest, ManifestReplacementWithUnassignedSnapshotId) {
  ASSERT_THAT(AppendFiles({file_a_, file_b_}), IsOk());
  ICEBERG_UNWRAP_OR_FAIL(auto original_snapshot, table_->current_snapshot());

  for (bool inherit_snapshot_id : {false, true}) {
    SCOPED_TRACE(inherit_snapshot_id);
    if (inherit_snapshot_id) {
      SetSnapshotIdInheritanceEnabled();
    }

    ICEBERG_UNWRAP_OR_FAIL(auto before, CurrentManifests());
    ASSERT_EQ(before.size(), 1U);
    ICEBERG_UNWRAP_OR_FAIL(auto entries, ReadManifestEntries(before[0]));
    auto path = std::format("{}/metadata/unassigned-{}.avro", table_location_,
                            inherit_snapshot_id);
    ICEBERG_UNWRAP_OR_FAIL(
        auto writer, ManifestWriter::MakeWriter(
                         format_version(), std::nullopt, path, file_io_, spec_, schema_,
                         ManifestContent::kData, before[0].first_row_id));
    for (const auto& entry : entries) {
      ASSERT_THAT(writer->WriteExistingEntry(entry), IsOk());
    }
    ASSERT_THAT(writer->Close(), IsOk());
    ICEBERG_UNWRAP_OR_FAIL(auto replacement, writer->ToManifestFile());
    ASSERT_FALSE(replacement.added_snapshot_id.has_value());

    auto assigned_manifest = replacement;
    assigned_manifest.added_snapshot_id = original_snapshot->snapshot_id;
    ICEBERG_UNWRAP_OR_FAIL(auto invalid_rewrite, table_->NewRewriteManifests());
    invalid_rewrite->DeleteManifest(before[0]).AddManifest(assigned_manifest);
    EXPECT_THAT(invalid_rewrite->Commit(),
                HasErrorMessage("Snapshot id must be assigned during commit"));

    ICEBERG_UNWRAP_OR_FAIL(auto rewrite, table_->NewRewriteManifests());
    rewrite->DeleteManifest(before[0]).AddManifest(replacement);
    ASSERT_THAT(rewrite->Commit(), IsOk());

    ICEBERG_UNWRAP_OR_FAIL(auto manifests, CurrentManifests());
    ASSERT_EQ(manifests.size(), 1U);
    ICEBERG_UNWRAP_OR_FAIL(auto snapshot, table_->current_snapshot());
    EXPECT_NE(snapshot->snapshot_id, original_snapshot->snapshot_id);
    EXPECT_EQ(manifests[0].added_snapshot_id, snapshot->snapshot_id);
    if (inherit_snapshot_id || format_version() > 1) {
      EXPECT_EQ(manifests[0].manifest_path, replacement.manifest_path);
    } else {
      EXPECT_NE(manifests[0].manifest_path, replacement.manifest_path);
    }
    ExpectManifestEntriesWithSnapshotIds(
        manifests[0], {file_a_->file_path, file_b_->file_path},
        {ManifestStatus::kExisting, ManifestStatus::kExisting},
        {original_snapshot->snapshot_id, original_snapshot->snapshot_id});
  }
}
...
TEST_P(RewriteManifestsTest, RebuildsRewriteAndReplacementAfterCommitConflict) {
  ASSERT_THAT(AppendFiles({file_a_}), IsOk());
  ICEBERG_UNWRAP_OR_FAIL(auto first_snapshot, table_->current_snapshot());
  ICEBERG_UNWRAP_OR_FAIL(auto first_manifests, CurrentManifests());
  ASSERT_EQ(first_manifests.size(), 1U);
  ...
  ICEBERG_UNWRAP_OR_FAIL(
      auto replacement,
      WriteExistingManifest("retry-replacement", first_snapshot->snapshot_id, {file_a_}));

  SetCommitRetryProperties(1);
  BindTableWithFailingCommits(/*failures=*/1);

  std::unordered_set<std::string> deleted_paths;
  ICEBERG_UNWRAP_OR_FAIL(auto rewrite, table_->NewRewriteManifests());
  rewrite->DeleteManifest(first_manifests[0])
      .AddManifest(replacement)
      .ClusterBy([](const DataFile&) { return "file"; })
      .Set("retry-property", "retained")
  ...
  EXPECT_FALSE(deleted_paths.empty());
  EXPECT_FALSE(deleted_paths.contains(replacement.manifest_path));
  EXPECT_TRUE(FileExists(replacement.manifest_path));
  ...
  ASSERT_EQ(manifests.size(), 2U);
  for (const auto& manifest : manifests) {
    EXPECT_FALSE(deleted_paths.contains(manifest.manifest_path));
  ...
  ICEBERG_UNWRAP_OR_FAIL(auto snapshot, table_->current_snapshot());
  ValidateSummary(snapshot, /*replaced=*/2, /*kept=*/0, /*created=*/2,
                  /*entry_count=*/1);
  EXPECT_THAT(snapshot->summary,
              ::testing::Contains(::testing::Pair("retry-property", "retained")));
}
